...
for (let index = 0; index < (stream.length - 14); index++) {
    const is4LettersUnique = stream
        .slice(index, index + 14) // Section of 4 Letters
        .split("") // Split into characters
        .filter((val, idx, arr) => arr.lastIndexOf(val) != idx).length == 0; // Check if there's repeated characters
    if (is4LettersUnique) {
        console.log(index + 14, stream.slice(index, index + 14));
        break;
    }
}
